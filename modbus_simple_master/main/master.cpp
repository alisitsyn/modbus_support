...
void app_main()
{
    ...
    ESP_ERROR_CHECK(cModbusMaster.begin(MB_SLAVE_SHORT_ADDRESS, &comm));
    ...
    for (int i = 0; i < MB_RETRIES; i++) {
        err = cModbusMaster.writeMultipleRegisters(0, 5, &reg_array[0]);
        MASTER_CHECK((err == ESP_OK), ; ,
                    "Modbus Write error: (0x%x).", (uint32_t)err);

        memset(reg_array, 0x00, 10);
        err = cModbusMaster.readHoldingRegisters(0, 5, &reg_array[0]);
        MASTER_CHECK((err == ESP_OK), ; ,
                    "Modbus Read error: (0x%x).", (uint32_t)err);

        temp_coils ^= 0xAAAA;

        err = cModbusMaster.writeMultipleCoils(0, 10, &temp_coils);
        MASTER_CHECK((err == ESP_OK), ; ,
                            "Modbus Write Coils error: (0x%x).", (uint32_t)err);

        err = cModbusMaster.readCoils(0, 10, &temp_coils);
        MASTER_CHECK((err == ESP_OK), ; ,
