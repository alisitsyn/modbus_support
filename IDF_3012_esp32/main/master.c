...
const mb_parameter_descriptor_t device_parameters[] = {
    ...
    { CID_HOLD_DATA_0, STR("Humidity_1"), STR("%rH"), MB_DEVICE_ADDR1, MB_PARAM_HOLDING, 0, 2,
            HOLD_OFFSET(holding_data0), PARAM_TYPE_FLOAT, 4, OPTS(0, 100, 1), PAR_PERMS_READ_WRITE },
    { CID_INPUT_DATA_0, STR("Input_1"), STR("C"), MB_DEVICE_ADDR1, MB_PARAM_INPUT, 0, 2,
                        INPUT_OFFSET(input_data0), PARAM_TYPE_FLOAT, 4, OPTS(0, 100, 1), PAR_PERMS_READ_WRITE },
};
...
void mb_master_task(void *para)
{
    ...
    // with the Slave with following restart of Master port
    while(1){
        mb_master_init();
        printf("in serial master task\n");
