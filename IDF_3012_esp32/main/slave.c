...
#define MB_SLAVE_PORT_NUM       (CONFIG_MB_SLAVE_UART_PORT_NUM)   // Number of UART port used for Modbus connection
#define MB_SLAVE_ADDR           (CONFIG_MB_SLAVE_ADDR) // The address of device in Modbus network
#define MB_SLAVE_DEV_SPEED      (CONFIG_MB_SLAVE_UART_BAUD_RATE)  // The communication speed of the UART
#define MB_SLAVE_UART_RXD       (CONFIG_MB_SLAVE_UART_RXD)
#define MB_SLAVE_UART_TXD       (CONFIG_MB_SLAVE_UART_TXD)
#define MB_SLAVE_UART_RTS       (CONFIG_MB_SLAVE_UART_RTS)
#define MB_SLAVE_MODE_RTU       (CONFIG_MB_COMM_MODE)
...
esp_err_t mb_slave_init(void)
{
    ...
    comm_info.port = MB_SLAVE_PORT_NUM;
    comm_info.baudrate = MB_SLAVE_DEV_SPEED;
    ...
    ESP_ERROR_CHECK(uart_set_pin(MB_SLAVE_PORT_NUM, MB_SLAVE_UART_TXD,
                            MB_SLAVE_UART_RXD, MB_SLAVE_UART_RTS,
                            UART_PIN_NO_CHANGE));
...
void slave_operation_func(void *para)
{
...
{
    // This demonstrates several communication sessions
    // with the master with following restart of slave port
    while(1){
        mb_slave_init();
        printf("In serial slave task\n");
