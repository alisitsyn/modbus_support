...
static MbSlaveInfo_t* vMBTCPPortMasterGetCurrInfo()
{
...
static BOOL xMBTCPPortMasterCheckHost(const CHAR* pcHostStr, ip_addr_t* pxHostAddr)
{
    ...
    ESP_LOGD(MB_TCP_MASTER_PORT_TAG, "Host[IP]: \"%s\"[%s]", pxAddrList->ai_canonname, pcStr);
...
static void vMBTCPPortMasterTask(void *pvParameters)
{
    ...
    while (1) {
        MbSlaveAddrInfo_t xSlaveAddrInfo = { 0 };
        BaseType_t xStatus = xQueueReceive(xMbPortConfig.xConnectQueue, (void*)&xSlaveAddrInfo, portMAX_DELAY);
        if (xStatus != pdTRUE) {
