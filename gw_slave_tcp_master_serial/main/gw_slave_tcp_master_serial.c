...
// This implementation is able to read/write requests from external TCP Master and translate them to the slave devices connected into Modbus segment. 
// The modbus data dictionary is not used but the mapping areas are defined in the Modbus TCP to reflect the values in its memory (this can be disabled).
// The Gateway gets requests from Slave TCP instance and translates the request to installed Modbus Master instance previously configured.
// The translation and mapping teqnique uses the approach with wrapped callback read/write functions to make the adapter to translate the data between instances. 
// This example is prepared from scratch to just demonstrate possible approach for the gateway. 
// Other approaches can be used to override the component sources and realize the gateway object for data translation. 
// The adapter functionality is located in mb_lib library which exposes some internals of installed modbus library.
...
#define MB_CHAN_DATA_MAX_VAL                (100)
...
// Mimic the data dictionary (it is needed to initialize the master but will not be used)
const mb_parameter_descriptor_t device_par_dummy[] = {
{ 
    0, (char*)"Dummy_key", (char*)"Dummy_unit", 0, 
    MB_PARAM_INPUT, 0, 1, 0, 
    PARAM_TYPE_FLOAT, 4, OPTS( 0, 0, 0 ), PAR_PERMS_READ_WRITE_TRIGGER }
};

// Modbus master initialization
static esp_err_t master_serial_init(void)
{
    // Initialize and start Modbus controller
    mb_communication_info_t comm = {
            .port = CONFIG_MB_UART_PORT_NUM,
    ...
    err = uart_set_pin(CONFIG_MB_UART_PORT_NUM, CONFIG_MB_UART_TXD, CONFIG_MB_UART_RXD,
    ...
    err = uart_set_mode(CONFIG_MB_UART_PORT_NUM, UART_MODE_RS485_HALF_DUPLEX);
    MB_RETURN_ON_FALSE((err == ESP_OK), ESP_ERR_INVALID_STATE, TAG,
            "mb serial set mode failure, uart_set_mode() returned (0x%x).", (int)err);

    err = mbc_master_set_descriptor(&device_par_dummy[0], 1);
    MB_RETURN_ON_FALSE((err == ESP_OK), ESP_ERR_INVALID_STATE, TAG,
...
static esp_err_t slave_tcp_init(mb_communication_info_t* comm_info)
{
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(slave_tcp_init(&comm_info));
    // Setup Modbus Serial Master
    ESP_ERROR_CHECK(master_serial_init());

    // The function is required if the gateway is still needs mapping of the read values,
