...
// Callback function for reading of MB Input Registers (Overriden)
eMBErrorCode eMBRegInputCBSerialSlave(UCHAR * pucRegBuffer, USHORT usAddress,
                                USHORT usNRegs)
{
    // <<<< The place to run user defined functionality of callback
    ESP_LOGI("USER INPUT CALLBACK", "RegStart: %d, Size: %d", usAddress, usNRegs);
...
esp_err_t mbc_serial_slave_create(void** handler)
{
    ...
    mbs_interface_ptr->slave_reg_cb_input = eMBRegInputCBSerialSlave; // override callback function for input registers
