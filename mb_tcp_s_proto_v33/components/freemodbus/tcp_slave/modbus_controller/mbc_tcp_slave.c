...
static esp_err_t mbc_tcp_slave_setup(void* comm_info)
{
    ...
    MB_SLAVE_CHECK(((comm_settings->tcp_addr_type == MB_TCP_IPV4) || (comm_settings->tcp_addr_type == MB_TCP_IPV6)),
                    ESP_ERR_INVALID_ARG, "mb incorrect addr type = (0x%x).", (uint8_t)comm_settings->tcp_addr_type);
