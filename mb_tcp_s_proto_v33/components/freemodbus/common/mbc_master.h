...
#include "freertos/portmacro.h"     // for mutex defines
...
#define MB_MASTER_CHECK_IFACE_FUNC(iface, func) do { \
        MB_MASTER_ASSERT(iface != NULL); \
        MB_MASTER_ASSERT(iface->func != NULL); \
    } while(0)
 ...
 */
typedef struct {
    portMUX_TYPE* mbm_mutex;                            /*!< Modbus port mutex */
