...
extern mb_descr_entry_t* mbc_slave_find_reg_descriptor(mb_param_type_t type, uint16_t addr, size_t regs);
extern eMBErrorCode mbc_reg_holding_slave_cb(UCHAR * reg_buffer, USHORT address, USHORT n_regs, eMBRegisterMode mode);

eMBErrorCode mbc_serial_reg_holding_slave_cb(UCHAR * reg_buffer, USHORT address, USHORT n_regs, eMBRegisterMode mode)
{
    ...
    if (it != NULL) {
        ...
        // reg_buffer - points to source data from frame to be written into location
        // holding_buffer - the pointer to the memory location to be written
        if (mode == MB_REG_WRITE) {
            if (it->access_ro) {
                ESP_LOGE("Custom CB", "Can not write to RO area starting %p:%d, %x", 
                                                    holding_buffer, n_regs, *(uint16_t*)reg_buffer);
                return MB_EIO;
            }
        }
        status =  mbc_reg_holding_slave_cb(reg_buffer, address, n_regs, mode);
    } else {
        status = MB_ENOREG;
    }
    return status;
...
esp_err_t mbc_serial_slave_create(void** handler)
{
    ...
    mbs_interface_ptr->slave_reg_cb_holding = mbc_serial_reg_holding_slave_cb;
